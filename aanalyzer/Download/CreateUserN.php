...
function buscaEval_Customer ($usernameDB) {
    $dbh = $dbh = mysqli_connect('localhost', 'analizer', 'anapass', 'aanalizer');
    $sql = "select count(customerid) from customer where username='$usernameDB';";
    $result = mysqli_query($dbh, $sql);
    $row = mysqli_fetch_array($result, MYSQLI_BOTH);
    if (!$result) {
...
if ($_POST["recaptcha_response_field"]) {
    ...
    if ($resp->is_valid) {
        ...
        $dbh = mysqli_connect('localhost', 'analizer', 'anapass', 'aanalizer');
        if (!$dbh) {
            die("Error in connection: " . mysqli_error($dbh));
        }
        // escape strings in input data
        $firstname = mysqli_real_escape_string($dbh, $_POST['cfirstname']);
        ...
        $phone = mysqli_real_escape_string($dbh, $_POST['cphone']);
//               $username = mysqli_real_escape_string($dbh, $_POST['cusername']);
//               $password = mysqli_real_escape_string($dbh, $_POST['cpassword']);
        $organization =  mysqli_real_escape_string($dbh, $_POST['corganization']);
        ...
        // execute query
        $sql = "INSERT  into customer (firstname,lastname,address1,address2,city,state,zip,country,email,phone,username,password,organization,userfullname
,product) VALUES ( '$firstname' ,'$lastname','$address1','$address2','$city','$state','$zip','$country','$email','$phone','$usernameDB','$passwordDB','$organization', '$firstname $lastname', 'iAAnalyzerTRIAL.exe')";
        $result = mysqli_query($dbh, $sql);
        if (!$result) {
